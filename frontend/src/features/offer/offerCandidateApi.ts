...
export const getMyOffers = (applicationId?: number) => axiosClient.get<OfferResponse[]>("/offer/offers", { params: applicationId ? { applicationId } : {} });
export const getOfferById = (id: number) => axiosClient.get<OfferResponse>(`/offer/offers/${id}`);
export const acceptOffer = (id: number) => axiosClient.patch<OfferResponse>(`/offer/offers/${id}/accept`);
